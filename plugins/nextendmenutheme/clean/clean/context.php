...
$border = NextendParse::parse($data->get('border'));
$borderopacity = substr($border,6,2);
$borderrgba = NextendColor::hex2rgba($border);
if($borderopacity!='00'){
  $context['border'] = '1px solid rgba('.$borderrgba[0].','.$borderrgba[1].','.$borderrgba[2].','.round($borderrgba[3]/127, 2).')';
} else {
  $context['border'] = 'none';
}

$shadow = NextendParse::parse($data->get('shadow'));
if($shadow==0){
  $context['shadow'] = '0';
} else {
  $context['shadow'] = '0 0 5px #bbb';
}
