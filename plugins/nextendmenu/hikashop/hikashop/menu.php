...
class NextendTreeHikashop extends NextendTreebaseJoomla {
    ...
    function initConfig() {
        ...
        $this->_config['onlyproducts']  = intval($this->_data->get('onlyproducts', 0));
        $this->_config['productview']   = intval($this->_data->get('productview', 0));
    ...
    function getAllItems() {
        ...
        foreach ($menus AS $menu) {
            $menuid = intval(substr($menu->config_namekey, 5));
            if (!intval($menuid)) continue;
            $d = unserialize(base64_decode($menu->config_value));
            if ($d['content_type'] != 'category' || !isset($itemss[$menuid])) continue;

            if ($menuid && $d['selectparentlisting']) {
                $this->itemids[$d['selectparentlisting']] = $menuid;
            }
        }
...
WHERE ((category_published =1 AND category_type='product') OR (category_type='root' AND category_published =0)) ";
        ...
        $a = array();
        hikashop_addACLFilters($a, 'category_access', '');
        if (!empty($a[0])) {
            $query .= ' AND ' . $a[0];
        }
...
a.category_id AS parent, 'prod' AS typ, 0 AS productnum, '' AS itemid, '' AS description FROM #__hikashop_product_category AS a LEFT JOIN #__hikashop_product AS b ON a.product_id = b.product_id WHERE b.product_published = 1 ";
        ...
        if ($this->_config['onlyproducts']) {
            foreach ($allItems AS $key => $value) {
                if (in_array($allItems[$key]->parent, $this->_config['root'])) {
    ...
    function getItemsTree() {
        ...
        for ($i = 0; $i < count($items); $i++) {
            if (isset($this->itemids[$items[$i]->id])) {
                $items[$i]->itemid = $this->itemids[$items[$i]->id];
            } else {
                $items[$i]->itemid = isset($items[$i]->parent->itemid) ? $items[$i]->parent->itemid : null;
            }
        }
    ...
    function filterItem($item) {
        ...
        //$ii = $item->itemid ? $item->itemid : ($this->_config['itemid'] ? $this->_config['itemid'] : null);         
        $ii = $this->_config['itemid'] ? $this->_config['itemid'] : ($item->itemid ? $item->itemid : null);
        $ii = $ii ? '&Itemid=' . $ii : '';
        if ($item->typ == 'cat') {
            if (!$this->_config['parentlink'] && $item->p) {
                $item->nname = '<a>' . $item->nname . '</a>';
            } else {
                if ($this->_config['productview']) {
                    $item->nname = '<a href="' . hikashop_completeLink('product&task=listing&cid=' . $item->id . '&name=' . $this->getCategoryAlias($item) . $ii) . '">' . $item->nname . '</a>';
                } else {
                    $item->nname = '<a href="' . hikashop_completeLink('category&task=listing&cid=' . $item->id . '&name=' . $this->getCategoryAlias($item) . $ii) . '">' . $item->nname . '</a>';
                }
            }
        } elseif ($item->typ == 'prod') {
            $id          = explode("-", $item->id);
            $item->nname = '<a href="' . hikashop_completeLink('product&task=show&cid=' . $id[1] . '&name=' . $this->getProductAlias($item) . ($this->_config['categoryp'] ? '&categoryp=' . $id[0] : '') . $ii) . '">' . $item->nname . '</a>';
        }
    }
