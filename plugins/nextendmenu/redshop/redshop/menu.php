...
class NextendTreeRedshop extends NextendTreebaseJoomla {
    ...
    function initConfig() {
        ...
        $this->_config['itemid'] = $this->_data->get('itemid', JRequest::getInt('Itemid'));
    ...
    function getAllItems() {
        ...
        if (!$this->_config['rootasitem'] && count($this->_config['root']) == 1) {
            ...
            $query .= "IF(a.id in (" . implode(',', $this->_config['root']) . "), 0 , IF(a.id = 0, -1, a.id)) AS parent, ";
        } else {
        ...
        if ($this->_config['order'] == "asc") {
            ...
            $query .= "ORDER BY a.ordering ASC, a.name ASC";
        }
    ...
    function filterItem($item) {
        ...
        if ($item->typ == 'cat') {
            if (!$this->_config['parentlink'] && $item->p) {
                ...
                $item->nname = '<a href="' . JRoute::_('index.php?option=com_redshop&view=category&cid=' . $item->id . '&layout=detail&Itemid=' . $this->_config['itemid']) . '">' . $item->nname . '</a>';
            }
        } elseif ($item->typ == 'prod') {
            $id          = explode("-", $item->id);
            $item->nname = '<a href="' . JRoute::_('index.php?option=com_redshop&view=product&pid=' . $id[1] . '&cid=' . $id[0] . '&Itemid=' . $this->_config['itemid']) . '">' . $item->nname . '</a>';
        }
    }
